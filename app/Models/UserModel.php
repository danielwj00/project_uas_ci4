...
class UserModel extends Model
{
    ...
    function editUserName($id_user, $name, $birthdate, $id_role)
    {
        $db = \Config\Database::connect('default');
        $db->transBegin();
        $db->query('UPDATE user set `name`=?, birthdate=? where `id_user`=?', [$name, $birthdate, $id_user]);
        if ($id_role == 'R0002') {
            $db->query('UPDATE guru set `name`=? where `id_user`=?', [$name, $id_user]);
        } else if ($id_role == 'R0003') {
            $db->query('UPDATE murid set `name`=? where `id_user`=?', [$name, $id_user]);
        }
        if ($db->transStatus() === FALSE) {
            $db->transRollback();
            return false;
    ...
    {
        $email = session()->get('sekolah.project_uas.email');
    ...
    function addMurid($id_user, $name)
    {
        $lastMuridId = $this->getLastMuridId();
        if (!$lastMuridId) {
            $id_murid = 'M0001';
        } else {
            $number_murid = substr($lastMuridId, 1);
            $int_murid = intval($number_murid) + 1;
            $int_length = strlen((string) $int_murid);
            if ($int_length == 4) {
                $id_murid = 'M' . $int_murid;
            } else if ($int_length == 3) {
                $id_murid = 'M0' . $int_murid;
            } else if ($int_length == 2) {
                $id_murid = 'M00' . $int_murid;
            } else if ($int_length == 1) {
                $id_murid = 'M000' . $int_murid;
            }
        }
        $db = \Config\Database::connect('default');
        $db->transBegin();

        $db->query('INSERT into murid(id_murid, id_user, name)
        values(?,?,?)', [$id_murid, $id_user, $name]);
        if ($db->transStatus() === FALSE) {
            $db->transRollback();
            return false;
        } else {
    ...
    function getLastMuridId()
    {
        $query = $this->db->query('SELECT * from murid order by 1 desc limit 1');
        $data = $query->getRowArray();
    ...
    {
        if ($id === false) {
            $query = $this->db->query('SELECT u.*, m.id_murid from user as u 
            ...
            $query = $this->db->query('SELECT u.*, m.id_murid from user as u 
            inner join murid as m
            on m.id_user=u.id_user where id_murid=?', [$id]);
            return $query->getRowArray();
        }
    }
    function deleteMurid($id)
    {
        $sql = 'DELETE from murid where id_user=?';
        $this->db->query($sql, [$id]);
    }
    ...
    function addKelas($name)
    {
        ...
        if (!$lastKelasId) {
            ...
            $number_kelas = substr($lastKelasId, 1);
            $int_kelas = intval($number_kelas) + 1;
            $int_length = strlen((string) $int_kelas);
            if ($int_length == 4) {
                $id_kelas = 'K' . $int_kelas;
            } else if ($int_length == 3) {
                $id_kelas = 'K0' . $int_kelas;
            } else if ($int_length == 2) {
                $id_kelas = 'K00' . $int_kelas;
            } else if ($int_length == 1) {
                $id_kelas = 'K000' . $int_kelas;
            }
        }
        $db->transBegin();
    ...
    function getLastAlokasiKelasId()
    {
        $query = $this->db->query('SELECT * from alokasi_kelas order by id_alokasi desc limit 1');
        $data = $query->getRowArray();
        if($data == null){
            return 0;
        }
        return $data['id_alokasi'];
    }
    function addAlokasiKelas($id_kelas, $id_murid, $id_guru, $id_mata_pelajaran)
    {
        $db = \Config\Database::connect('default');
        $db->transBegin();

        $lastIdAlokasi = $this->getLastAlokasiKelasId();
        $newIdAlokasi = $lastIdAlokasi + 1;

        $db->query('INSERT into alokasi_kelas(id_alokasi, id_kelas, id_murid, id_guru, id_mata_pelajaran)
        values(?,?,?,?,?)', [$newIdAlokasi, $id_kelas, $id_murid, $id_guru, $id_mata_pelajaran]);
        if ($db->transStatus() === FALSE) {
            $db->transRollback();
            return false;
        } else {
    ...
    function checkAddAlokasiKelas($id_kelas, $id_murid, $id_guru, $id_mata_pelajaran){
        $sql = 'SELECT * from alokasi_kelas where id_kelas=? and id_murid=? and id_guru=? and id_mata_pelajaran=?';
        $query = $this->db->query($sql, [$id_kelas, $id_murid, $id_guru, $id_mata_pelajaran]);
    ...
    function deleteAlokasiKelas($id)
    {
        $db = \Config\Database::connect('default');
        $db->transBegin();

        $db->query('DELETE from alokasi_kelas where id_alokasi=?', [$id]);
        if ($db->transStatus() === FALSE) {
            $db->transRollback();
            return false;
        } else {
    ...
    function editUserProfileWithPhoto($id, $name, $email, $password, $birthdate, $nama_foto)
    {
        ...
        $sql = 'UPDATE user set name=?, email=?, password=?, birthdate=?, link_foto=? where id_user=?';
        $db->query($sql, [$name, $email, $password, $birthdate, $nama_foto, $id]);
    ...
    function editUserProfileNoPasswordWithPhoto($id, $name, $email, $birthdate, $nama_foto)
    {
        ...
        $sql = 'UPDATE user set name=?, email=?, birthdate=?, link_foto=? where id_user=?';
        $db->query($sql, [$name, $email, $birthdate, $nama_foto, $id]);
    ...
    function addMataPelajaran($name)
    {
        ...
        if (!$lastMataPelajaranId) {
            ...
            if ($int_length == 4) {
                $id_mata_pelajaran = 'MP' . $int_mata_pelajaran;
            } else if ($int_length == 3) {
                $id_mata_pelajaran = 'MP0' . $int_mata_pelajaran;
            } else if ($int_length == 2) {
                $id_mata_pelajaran = 'MP00' . $int_mata_pelajaran;
            } else if ($int_length == 1) {
                $id_mata_pelajaran = 'MP000' . $int_mata_pelajaran;
            }
        }
        $db->transBegin();
    ...
    function askEditProfilePermission($id_user){
        ...
        $message = $user_name.'['.$id_user.']'.' asking for edit profile permission';
    ...
    function clearNotificationByIdNotificationTeacher($id_notification){
        $db = \Config\Database::connect('default');
